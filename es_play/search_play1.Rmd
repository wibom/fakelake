...
get_individuals_in_dataset <- function(filename) {
  # To achieve double escaped forward slashes (necessary for use in ES-query),
  # we need quadruple escape as input to `glue::glue`
  quadruple_escaped_path <- filename %>% str_replace_all("/", "\\\\\\\\/")
  body <- glue(
    '{{
      "size": 0,
      "query": {{
...
>
> * Query returns all 30 individuals as included in each file
> * Both `x1` files contain only individuals 1-20
> * The `x2` file contains only individuals 1-25


Must figure out how to re-formulate query to get accurate result.
